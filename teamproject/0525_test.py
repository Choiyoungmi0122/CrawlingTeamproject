...
driver = webdriver.Chrome('./chromedriver', options=chrome_options)  # 드라이버 경로
...
# 주차장 리스트
parking_list = driver.find_elements(By.CSS_SELECTOR, 'DWs4Q z23xh')
...
# 크롤링 (페이지 리스트 만큼)
for btn in range(1):  # next_btn[0] = 이전 페이지 버튼 무시 -> [1]부터 시작
    parking_list = driver.find_elements(By.CSS_SELECTOR, 'DWs4Q z23xh')
    names = driver.find_elements(By.CSS_SELECTOR, '.place_bluelink q2LdB')  # (3) 장소명
    ...
    for data in range(len(parking_list)):  # 주차장 리스트 만큼
        ...
        try:
            ...
            # (5) 주소 버튼 누르기
            address_buttons = driver.find_elements(By.CSS_SELECTOR, '.berX5')
            ...
            addr = driver.find_elements(By.CSS_SELECTOR, '.nvb9q > div')
