...
for btn in range(2): 
    ...
    for data in range(len(parking_list)):  #장소 리스트 만큼
        ...
        try:
            ...
            # (8) 상세정보 버튼 누르기 
            infor_buttons = driver.find_element(By.CSS_SELECTOR, 'z23xh > a')
            infor_buttons.__getitem__(data).click()
            sleep(1)

            # (9) 상세정보 눌렀을 때 도로명, 지번 나오는 div
            infor = driver.find_elements(By.CSS_SELECTOR, '.BXtr_ tAvTy > div')
